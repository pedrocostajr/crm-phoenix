...
serve(async (req) => {
  ...
  try {
    ...
    const { data: profileData, error: adminProfileError } = await supabaseAdmin
      .from('profiles')
      .select('role')
      .eq('user_id', userData.user.id)
      .single()

    if (adminProfileError || profileData?.role !== 'admin') {
      console.error('❌ Insufficient permissions:', {
        email: userData.user.email,
        role: profileData?.role,
        error: adminProfileError
      })
      throw new Error(`Permissões insuficientes. Apenas administradores podem criar usuários.`)
    }
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      email: email,
      password: requestBody.password || 'temp123456', // Usar senha fornecida ou padrão
      email_confirm: true,
      user_metadata: {
        full_name: full_name,
      }
